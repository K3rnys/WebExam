...
from models import db, User, Role, Book, Genre, Cover, Review, ReviewStatus, Collection, CollectionBook  
...
def sanitize_html(text):
    """Очищает и преобразует разметку Markdown в безопасный HTML с разрешением большинства тегов."""
    allowed_tags = [
        'p', 'div', 'br', 'hr',
        'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
        'blockquote', 'pre', 'code',
...
def index():
    ...
    if title:
        query = query.filter(func.lower_cyrillic(Book.title).like(f'%{title.casefold()}%'))
    if author:
        query = query.filter(func.lower_cyrillic(Book.author).like(f'%{author.casefold()}%'))
    if genre_ids:
...
def collection_view(collection_id):
    ...
    return render_template('collection_view.html', collection=collection, books=books)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        def register_lower_cyrillic(dbapi_connection, connection_record):
            def lower_cyrillic(s):
                if s is None:
                    return s
                if not isinstance(s, str):
                    try:
                        ...
                        s = str(s)
                return s.casefold()
            dbapi_connection.create_function("lower_cyrillic", 1, lower_cyrillic)
